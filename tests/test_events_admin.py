...
async def _run_step(fn_name, text, user_data=None, evdata=None):
    ...
    with patch("storage.get_all_events_data", side_effect=_get), \
        ...
        result = await getattr(ea, fn_name)(upd, ctx)
...
class TestAddEventFlow:
    async def test_non_admin_blocked(self):
        ...
        with patch("permissions.is_admin", return_value=False):
            result = await ea.cmd_addevent(_upd(), _ctx())
        assert result == ConversationHandler.END

    async def test_entry_asks_for_name(self):
        import handlers.events_admin as ea
        ctx, upd = _ctx(), _upd()
        with patch("permissions.is_admin", return_value=True):
            result = await ea.cmd_addevent(upd, ctx)
        assert result == ea.AE_NAME
        assert "name" in upd.message.reply_text.call_args[0][0].lower()

    async def test_name_advances_to_date(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("ae_name", "  Youth Night  ")
        assert result == ea.AE_DATE
        assert ctx.user_data["ae_name"] == "Youth Night"

    async def test_bad_time_format_stays(self):
        import handlers.events_admin as ea
        result, upd, ctx, _ = await _run_step("ae_time", "7pm")
        assert result == ea.AE_TIME
        assert "ae_time" not in ctx.user_data

    async def test_valid_time_advances(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("ae_time", "19:00")
        assert result == ea.AE_DURATION
        assert ctx.user_data["ae_time"] == "19:00"

    async def test_duration_defaults_when_not_a_number(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("ae_duration", "skip")
        assert result == ea.AE_DESC
        assert ctx.user_data["ae_duration"] == 60

    async def test_duration_uses_entered_number(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("ae_duration", "45")
        assert result == ea.AE_DESC
        assert ctx.user_data["ae_duration"] == 45

    async def test_dash_clears_description_and_url(self):
        import handlers.events_admin as ea
        _, _, ctx1, _ = await _run_step("ae_desc", "-")
        assert ctx1.user_data["ae_desc"] == ""
        result, _, ctx2, _ = await _run_step("ae_url", "-")
        assert result == ea.AE_NOTIF
        assert ctx2.user_data["ae_url"] == ""

    async def test_notif_defaults_and_shows_summary(self):
        import handlers.events_admin as ea
        base = {"ae_name": "Youth Night", "ae_date": _future_date(), "ae_time": "19:00",
                "ae_duration": 90, "ae_desc": "Bring a friend", "ae_url": ""}
        result, upd, ctx, _ = await _run_step("ae_notif", "notanumber", base)
    ...
    async def test_confirm_no_discards(self):
        from telegram.ext import ConversationHandler
        result, upd, _, saved = await _run_step("ae_confirm", "no",
            self._confirm_data(_future_date()))
        assert result == ConversationHandler.END
        assert "data" not in saved  # nothing written

    async def test_confirm_saves_one_time_event(self):
        from telegram.ext import ConversationHandler
        date = _future_date()
        result, _, _, saved = await _run_step("ae_confirm", "yes", self._confirm_data(date))
        assert result == ConversationHandler.END
    ...
    async def test_confirm_saves_weekly_event(self):
        result, _, _, saved = await _run_step("ae_confirm", "yes", self._confirm_data("weekly:2"))
        ev = saved["data"]["special_events"][0]
        assert ev["type"] == "weekly"
        assert ev["weekday"] == 2
        assert "date" not in ev

    async def test_confirm_appends_without_dropping_existing(self):
        existing = {"special_events": [{"id": "keep_me", "name": "Existing"}]}
        _, _, _, saved = await _run_step("ae_confirm", "yes", self._confirm_data(_future_date()),
                                   evdata=existing)
        ids = [e["id"] for e in saved["data"]["special_events"]]
        assert "keep_me" in ids and len(ids) == 2

    async def test_confirm_schedules_notification(self):
        import handlers.events_admin as ea
        ctx = _ctx()
        ...
        with patch("storage.get_all_events_data", side_effect=_get), \
            ...
            await ea.ae_confirm(_upd("yes"), ctx)
        sched.assert_called()
...
class TestModifyEventFlow:
    ...
    async def test_non_admin_blocked(self):
        ...
        from telegram.ext import ConversationHandler
        with patch("permissions.is_admin", return_value=False):
            result = await ea.cmd_modifyevent(_upd(), _ctx())
        assert result == ConversationHandler.END

    async def test_entry_lists_specials(self):
        import handlers.events_admin as ea
        ctx, upd = _ctx(), _upd()
        ...
        with patch("permissions.is_admin", return_value=True), \
            ...
            result = await ea.cmd_modifyevent(upd, ctx)
    ...
    async def test_select_by_number(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("me_select", "1", {"me_specials": [self._special()]})
        assert result == ea.ME_FIELD
        assert ctx.user_data["me_event"]["id"] == "special_abc"

    async def test_select_by_id(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("me_select", "special_abc",
                                      {"me_specials": [self._special()]})
        assert result == ea.ME_FIELD

    async def test_select_unknown_stays(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("me_select", "nope", {"me_specials": [self._special()]})
        assert result == ea.ME_SELECT
        assert "me_event" not in ctx.user_data

    async def test_invalid_field_stays(self):
        import handlers.events_admin as ea
        result, upd, ctx, _ = await _run_step("me_field", "colour")
        assert result == ea.ME_FIELD
        assert "me_field" not in ctx.user_data

    async def test_valid_field_advances(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("me_field", "Duration")
        assert result == ea.ME_VALUE
        assert ctx.user_data["me_field"] == "duration"

    async def _value_step(self, field, value, ev=None):
        ev = ev or self._special()
        data = {"special_events": [dict(ev)]}
        return await _run_step("me_value", value,
                         {"me_field": field, "me_event": ev}, evdata=data)

    async def test_value_updates_int_field(self):
        from telegram.ext import ConversationHandler
        result, _, _, saved = await self._value_step("duration", "120")
        assert result == ConversationHandler.END
        assert saved["data"]["special_events"][0]["duration_minutes"] == 120

    async def test_value_updates_notification_int(self):
        _, _, _, saved = await self._value_step("notification", "30")
        assert saved["data"]["special_events"][0]["notification_minutes"] == 30

    async def test_value_coerces_active_bool(self):
        _, _, _, saved = await self._value_step("active", "no")
        assert saved["data"]["special_events"][0]["active"] is False
        _, _, _, saved2 = await self._value_step("active", "yes")
        assert saved2["data"]["special_events"][0]["active"] is True

    async def test_value_updates_string_field(self):
        _, _, _, saved = await self._value_step("name", "Youth Gathering")
        assert saved["data"]["special_events"][0]["name"] == "Youth Gathering"

    async def test_value_leaves_other_events_untouched(self):
        ev = self._special()
        other = {"id": "special_other", "name": "Other", "type": "once",
                 "date": _future_date(), "time": "08:00"}
        data = {"special_events": [dict(ev), other]}
        _, _, _, saved = await _run_step("me_value", "Renamed",
...
class TestDeleteEventFlow:
    ...
    async def test_non_admin_blocked(self):
        ...
        from telegram.ext import ConversationHandler
        with patch("permissions.is_admin", return_value=False):
            result = await ea.cmd_deleteevent(_upd(), _ctx())
        assert result == ConversationHandler.END

    async def test_entry_lists_upcoming(self):
        import handlers.events_admin as ea
        ctx, upd = _ctx(), _upd()
        ...
        with patch("permissions.is_admin", return_value=True), \
            ...
            result = await ea.cmd_deleteevent(upd, ctx)
        assert result == ea.DE_SELECT
        assert len(ctx.user_data["de_events"]) == 2
        assert "Youth Night" in upd.message.reply_text.call_args[0][0]

    async def test_invalid_selection_stays(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("de_select", "9",
                                      {"de_events": [self._special_ev()]})
        assert result == ea.DE_SELECT
        assert "de_ev" not in ctx.user_data

    async def test_special_selection_goes_to_confirm(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("de_select", "1",
                                      {"de_events": [self._special_ev()]})
        assert result == ea.DE_CONFIRM
        assert ctx.user_data["de_ev"]["key"] == "special_abc"

    async def test_convocation_selection_goes_to_annotation(self):
        import handlers.events_admin as ea
        result, _, ctx, _ = await _run_step("de_select", "1",
                                      {"de_events": [self._convocation_ev()]})
        assert result == ea.DE_ANNOT

    async def test_confirm_no_keeps_event(self):
        from telegram.ext import ConversationHandler
        data = {"special_events": [{"id": "special_abc", "name": "Youth Night"}]}
        result, _, _, saved = await _run_step("de_confirm", "no",
                                        {"de_ev": self._special_ev()}, evdata=data)
        assert result == ConversationHandler.END
        assert "data" not in saved

    async def test_confirm_yes_deletes_only_that_event(self):
        data = {"special_events": [{"id": "special_abc", "name": "Youth Night"},
                                   {"id": "special_keep", "name": "Other"}]}
        _, _, _, saved = await _run_step("de_confirm", "yes",
                                   {"de_ev": self._special_ev()}, evdata=data)
        ids = [e["id"] for e in saved["data"]["special_events"]]
        assert ids == ["special_keep"]

    async def test_annotation_dash_cancels(self):
        from telegram.ext import ConversationHandler
        result, _, _, saved = await _run_step("de_annot", "-",
                                        {"de_ev": self._convocation_ev()})
        assert result == ConversationHandler.END
        assert "data" not in saved

    async def test_annotation_appended_to_convocation(self):
        data = {"convocation_announcements": {}}
        _, _, _, saved = await _run_step("de_annot", "Service cancelled — icy roads",
    ...
    async def test_annotation_appends_to_existing_list(self):
        data = {"convocation_announcements": {"sabbath_eve_x": ["First note"]}}
        _, _, _, saved = await _run_step("de_annot", "Second note",
                                   {"de_ev": self._convocation_ev()}, evdata=data)
