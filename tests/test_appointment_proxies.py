...
class TestEnableCommand:
    async def _run_cmd(self, arg, from_id=999, from_username="pastor", officials=None):
        ...
        with patch("permissions.OFFICIALS", officials), \
            ...
            await ha.cmd_enable_appt_proxies(upd, ctx)
        return officials, upd, saved

    async def test_official_enables(self):
        officials, upd, saved = await self._run_cmd("yes")
        assert officials[0]["proxies_enabled"] is True
        assert saved["called"]

    async def test_official_disables(self):
        officials = [_official(enabled=True)]
        officials, upd, saved = await self._run_cmd("no", officials=officials)
        assert officials[0]["proxies_enabled"] is False

    async def test_non_official_rejected(self):
        officials, upd, saved = await self._run_cmd("yes", from_id=777, from_username="stranger")
        assert not saved["called"]
        assert "official" in upd.message.reply_text.call_args[0][0].lower()

    async def test_bad_arg_shows_usage(self):
        officials, upd, saved = await self._run_cmd("maybe")
        assert not saved["called"]
...
class TestClaim:
    async def _run(self, upd, appts, finalize=None):
    ...
    async def test_proxy_can_confirm(self):
        upd, q = _cb_update("appt:confirm:A1", from_id=222, from_username="janesec")
        saved, finalize, notif, ctx = await self._run(upd, [_appt()])
        finalize.assert_called_once()

    async def test_claim_records_negotiator(self):
    ...
    async def test_stale_actor_blocked(self):
        ...
        saved, finalize, notif, ctx = await self._run(upd, [claimed])
        finalize.assert_not_called()
        assert "already being handled" in q.edit_message_text.call_args[0][0].lower()

    async def test_same_negotiator_proceeds(self):
        upd, q = _cb_update("appt:confirm:A1", from_id=222, from_username="janesec")
        claimed = _appt(negotiator_chat_id=222, negotiator_name="Jane Sec",
                        negotiator_is_proxy=True)
        saved, finalize, notif, ctx = await self._run(upd, [claimed])
    ...
    async def test_unauthorized_actor_blocked(self):
...
class TestFinalizeProxyNote:
    async def _finalize(self, appt):
        ...
        with patch("storage.save_appointments", side_effect=_save), \
             ...
             patch("permissions.OFFICIALS", [_official(enabled=True)]):
            await ha._finalize_appointment(ctx, appt, [appt])
        return ctx

    async def test_proxy_gets_confirmation_note(self):
    ...
    async def test_no_proxy_note_when_official_handled(self):
        dt = (datetime.now(TZ) + timedelta(days=2)).isoformat()
        appt = _appt(status="confirmed", confirmed_datetime=dt,
...
class TestRequestFanout:
    async def test_request_sent_to_official_and_proxy(self):
        ctx = _ctx()
        appt = _appt()
    ...
    async def test_request_only_official_when_disabled(self):
        ctx = _ctx()
        appt = _appt()
