...
class TestPrefHelpers:
    ...
    async def test_get_user_prefs(self):
        ...
        with patch("storage.get_all_users", side_effect=_fake_get):
            tz, lang = await common.get_user_prefs(111)
        assert str(tz) == "America/Chicago"
        assert lang == "en"

    async def test_get_user_prefs_unknown_user_defaults(self):

        async def _fake_get():
            return []

        with patch("storage.get_all_users", side_effect=_fake_get):
            tz, lang = await common.get_user_prefs(999)
...
class TestSetTimezone:
    async def _run_select(self, text, users=None):
        ctx = _make_context()
        ...
        with patch("storage.get_all_users", side_effect=_fake_get), \
            ...
            result = await hub.tz_typed(upd, ctx)
        return result, upd, saved

    async def test_type_number_from_menu(self):
        result, upd, saved = await self._run_select("2")  # America/Chicago
        assert result == ext.ConversationHandler.END
        assert saved["users"][0]["timezone"] == "America/Chicago"

    async def test_type_custom_iana(self):
        result, upd, saved = await self._run_select("Europe/Paris")
        assert result == ext.ConversationHandler.END
        assert saved["users"][0]["timezone"] == "Europe/Paris"

    async def test_invalid_zone_stays(self):
        result, upd, saved = await self._run_select("Mars/Phobos")
        assert result == hub.TZ_SELECT
        assert "users" not in saved
        assert "recognised" in upd.message.reply_text.call_args[0][0].lower()

    async def test_confirmation_shows_zone(self):
        result, upd, saved = await self._run_select("America/Los_Angeles")
        assert "America/Los_Angeles" in upd.message.reply_text.call_args[0][0]


class TestSetTimezoneButtons:
    async def _run_button(self, data, users=None):
        ctx = _make_context()
        upd = MagicMock()
        ...
        with patch("storage.get_all_users", side_effect=_fake_get), \
             patch("storage.save_users", side_effect=_fake_save):
            result = await hub.tz_button(upd, ctx)
        return result, q, saved

    async def test_first_button_sets_new_york(self):
        result, q, saved = await self._run_button("tz:0")
        assert result == ext.ConversationHandler.END
        assert saved["users"][0]["timezone"] == "America/New_York"

    async def test_button_confirmation_shows_zone(self):
        _, q, _ = await self._run_button("tz:3")  # America/Los_Angeles
        assert "America/Los_Angeles" in q.edit_message_text.call_args[0][0]

    async def test_out_of_range_button_ends(self):
        result, q, saved = await self._run_button("tz:99")
        assert result == ext.ConversationHandler.END
        assert "users" not in saved

    async def test_cmd_settimezone_shows_buttons(self):
        ctx = _make_context()
        upd = _make_update()
        ...
        with patch("storage.get_all_users", side_effect=_fake_get):
            result = await hub.cmd_settimezone(upd, ctx)
        assert result == hub.TZ_SELECT
...
class TestLanguage:
    async def _run_select(self, data):
        ctx = _make_context()
        upd = MagicMock()
        upd.effective_user.id = 111
        upd.effective_user.username = "u"
        ...
        with patch("storage.get_all_users", side_effect=_fake_get), \
             patch("storage.save_users", side_effect=_fake_save):
            result = await hub.lang_select(upd, ctx)
        return result, q, saved

    async def test_select_english(self):
        result, q, saved = await self._run_select("lang:en")
        assert result == ext.ConversationHandler.END
        assert saved["users"][0]["language"] == "en"

    async def test_unknown_code_ends_without_saving(self):
        result, q, saved = await self._run_select("lang:xx")
        assert result == ext.ConversationHandler.END
        assert "users" not in saved
...
class TestEventsRespectUserTz:
    async def _run_events(self, user_record):
        ctx = _make_context()
        ...
        with patch("storage.get_all_users", side_effect=_fake_users), \
            ...
            await hub.cmd_events(upd, ctx)
        return upd.message.reply_text.call_args[0][0]

    async def test_eastern_user_sees_eastern_time(self):
        msg = await self._run_events({"chat_id": 111, "timezone": "America/New_York"})
        assert "10:00 AM" in msg

    async def test_pacific_user_sees_pacific_time(self):
        msg = await self._run_events({"chat_id": 111, "timezone": "America/Los_Angeles"})
