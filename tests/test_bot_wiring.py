...
async def _run_contact(upd, users=None, is_adm=False, known_official=False):
    import bot
    ctx = _ctx()
    saved = {}
...
class TestHandleContact:
    async def test_rejects_someone_elses_contact(self):
        saved, upd = await _run_contact(_contact_update(user_id=111, contact_user_id=222))
    ...
    async def test_registers_phone_and_official(self):
        saved, _ = await _run_contact(_contact_update())
        assert saved["registered_phone"][0] == 111
        assert saved["registered_official"][0] == 111

    async def test_stores_normalised_phone_on_user_record(self):
        saved, _ = await _run_contact(_contact_update(phone="+1 (555) 000-1234"))
        assert saved["users"][0]["phone"] == "15550001234"

    async def test_does_not_overwrite_existing_phone(self):
        users = [{"chat_id": 111, "phone": "9999999999"}]
        saved, _ = await _run_contact(_contact_update(), users=users)
        assert "users" not in saved                  # no save triggered
        assert users[0]["phone"] == "9999999999"     # left as-is

    async def test_unknown_user_record_is_not_created(self):
        saved, _ = await _run_contact(_contact_update(user_id=999), users=[{"chat_id": 111}])
        assert "users" not in saved

    async def test_admin_gets_admin_acknowledgement(self):
        saved, upd = await _run_contact(_contact_update(), is_adm=True)
        reply = upd.message.reply_text.call_args_list[0][0][0]
        assert "administrator" in reply.lower()

    async def test_official_gets_official_acknowledgement(self):
        saved, upd = await _run_contact(_contact_update(), known_official=True)
        reply = upd.message.reply_text.call_args_list[0][0][0]
        assert "official" in reply.lower()

    async def test_plain_user_gets_generic_thanks(self):
        saved, upd = await _run_contact(_contact_update())
        reply = upd.message.reply_text.call_args_list[0][0][0]
        assert "thank you" in reply.lower()

    async def test_admin_sees_admin_commands_listed(self):
        saved, upd = await _run_contact(_contact_update(), is_adm=True)
        cmd_text = upd.message.reply_text.call_args_list[1][0][0]
        assert "/addevent" in cmd_text           # admin-only command present

    async def test_regular_user_does_not_see_admin_commands(self):
        saved, upd = await _run_contact(_contact_update())
        cmd_text = upd.message.reply_text.call_args_list[1][0][0]
...
class TestErrorHandler:
    async def test_logs_error_to_activity_log(self):
        import bot
        ctx = _ctx()
        ctx.error = RuntimeError("boom")
        with patch.object(settings.activity, "log_error") as log_error:
            await bot.error_handler(MagicMock(), ctx)
        log_error.assert_called_once()
        assert "boom" in log_error.call_args[0][0]

    async def test_survives_non_exception_error_value(self):
        import bot
        ctx = _ctx()
...
class TestRescheduleJob:
    async def test_reschedules_all_upcoming(self):
        import bot
        ctx = _ctx()
        ctx.application = MagicMock()
        sched = AsyncMock()
        with patch("bot.schedule_all_upcoming", sched):
            await bot.reschedule_job(ctx)
...
class TestOfficialUsernameDriftAlert:
    async def _run_alert(self, uid=999, uname="pastornew", admin_users=None, officials=None):
        import bot
        ...
        with patch("storage.get_all_users", side_effect=_get_users), \
            ...
            await bot._warn_official_username_drift(ctx, uid, uname)
        return ctx

    async def test_alerts_admins(self):
        ctx = await self._run_alert()
    ...
    async def test_no_alert_when_no_drift(self):
        ctx = await self._run_alert(uname="pastorold")
        ctx.bot.send_message.assert_not_awaited()

    async def test_alert_is_sent_once_per_change(self):
        import bot
        ctx = await self._run_alert()
        ...
        with patch("storage.get_all_users", side_effect=_get_users), \
             ...
             patch.object(__import__("permissions"), "ADMIN_USERNAMES", {"alice"}), \
             patch.object(__import__("permissions"), "_admin_chat_ids", set()):
            await bot._warn_official_username_drift(ctx2, 999, "pastornew")
        ctx2.bot.send_message.assert_not_awaited()

    async def test_alert_repeats_for_a_different_new_username(self):
        import bot
        await self._run_alert(uname="pastornew")
        ctx2 = _ctx()
        ctx2.bot.send_message = AsyncMock()
        ...
        with patch("storage.get_all_users", side_effect=_get_users), \
             ...
             patch.object(__import__("permissions"), "ADMIN_USERNAMES", {"alice"}), \
             patch.object(__import__("permissions"), "_admin_chat_ids", set()):
            await bot._warn_official_username_drift(ctx2, 999, "pastorthird")
        ctx2.bot.send_message.assert_awaited_once()

    async def test_does_not_alert_the_person_who_changed_it(self):
        ...
        with patch("storage.get_all_users", side_effect=_get_users), \
             ...
             patch.object(__import__("permissions"), "_admin_chat_ids", set()):
            await bot._warn_official_username_drift(ctx, 999, "pastornew")
        sent_to = [c.args[0] for c in ctx.bot.send_message.await_args_list]
        assert 999 not in sent_to and sent_to == [1]

    async def test_markdown_in_names_is_escaped(self):
        offs = _officials_with(uname="old_name")
        offs[0]["name"] = "Pastor_Test *Snr*"
        ctx = await self._run_alert(officials=offs)
        msg = ctx.bot.send_message.await_args[0][1]
    ...
    async def test_send_failure_does_not_raise(self):
        ...
        with patch("storage.get_all_users", side_effect=_get_users), \
             ...
             patch.object(__import__("permissions"), "ADMIN_USERNAMES", {"alice"}), \
             patch.object(__import__("permissions"), "_admin_chat_ids", set()):
            await bot._warn_official_username_drift(ctx, 999, "pastornew")  # must not raise


class TestCommandHookTriggersDriftCheck:
    async def test_command_invocation_checks_for_drift(self):
        import bot
        upd = MagicMock()
        ...
        with patch("bot._refresh_user_identity", side_effect=_noop), \
            ...
            await bot._log_command_invocation(upd, _ctx())
...
class TestLogHousekeeping:
    async def test_post_init_clears_last_runs_error_log(self):
    ...
    async def test_daily_maintenance_prunes_runtime_log(self):
        import bot
...
class TestScheduledJobs:
    ...
    async def _built_app(self):
        ...
        with patch("bot.schedule_all_upcoming", side_effect=_noop_sched), \
            ...
            await built["post_init"](fake)
        return fake

    async def test_nightly_backup_scheduled_at_backup_time(self):
        import bot
        from handlers.backup import BACKUP_TIME, nightly_backup_job
        fake = await self._built_app()
    ...
    async def test_backup_command_registered(self):
        fake = await self._built_app()
