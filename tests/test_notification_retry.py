...
class TestDeliverEventNotifications:
    ...
    async def _deliver(self, bot, bot_obj, ctx_patches, event):
        for p in ctx_patches:
            p.start()
        try:
            return await hn.deliver_event_notifications(bot_obj, event)
        finally:
            for p in ctx_patches:
                p.stop()

    async def test_posts_once_per_target_group(self):
        bot, bot_obj, state, p = self._setup()
        sent = await self._deliver(bot, bot_obj, p, _event(targets=(-100, -200)))
        assert sent == 2
        assert bot_obj.send_message.await_count == 2

    async def test_no_targets_sends_nothing(self):
        bot, bot_obj, state, p = self._setup()
        sent = await self._deliver(bot, bot_obj, p, _event(targets=()))
        assert sent == 0
        bot_obj.send_message.assert_not_awaited()

    async def test_records_notified_targets_in_state(self):
        bot, bot_obj, state, p = self._setup()
        await self._deliver(bot, bot_obj, p, _event(key="abc", targets=(-100, -200)))
        assert set(state["states"]["abc"]["notified"]) == {-100, -200}

    async def test_does_not_repost_to_already_notified(self):
        bot, bot_obj, state, p = self._setup(
            state={"abc": {"name": "x", "service_time": "t", "notified": [-100]}},
        )
        sent = await self._deliver(bot, bot_obj, p, _event(key="abc", targets=(-100, -200)))
        assert sent == 1
        assert bot_obj.send_message.await_args[0][0] == -200

    async def test_fully_notified_is_noop(self):
        bot, bot_obj, state, p = self._setup(
            state={"abc": {"name": "x", "service_time": "t", "notified": [-100]}},
        )
        sent = await self._deliver(bot, bot_obj, p, _event(key="abc", targets=(-100,)))
        assert sent == 0
        bot_obj.send_message.assert_not_awaited()

    async def test_past_service_time_sends_nothing(self):
        bot, bot_obj, state, p = self._setup()
        sent = await self._deliver(bot, bot_obj, p, _event(minutes_to_service=-5))
        assert sent == 0
        bot_obj.send_message.assert_not_awaited()

    async def test_network_failure_leaves_target_pending(self):
    ...
    async def test_forbidden_leaves_target_pending(self):
    ...
    async def test_partial_failure_then_retry_completes(self):
        from telegram.error import NetworkError
        ...
        first = await self._deliver(bot, bot_obj, p, _event(key="abc", targets=(-100, -200)))
        assert first == 1
        assert state["states"]["abc"]["notified"] == [-100]

        second = await self._deliver(bot, bot_obj, p, _event(key="abc", targets=(-100, -200)))
        assert second == 1
        assert set(state["states"]["abc"]["notified"]) == {-100, -200}


class TestCatchupJob:
    async def test_delivers_for_in_window_event_and_prunes(self):
        ...
        with patch("handlers.notifications.all_upcoming", side_effect=_fake_all_upcoming), \
             ...
             patch("storage._load_notif_state", side_effect=_load_state), \
             patch("storage._save_notif_state", side_effect=_save_state):
            await hn.notification_catchup_job(ctx)
...
class TestCommandLogging:
    async def test_logs_command_at_info(self, caplog):
        import bot
    ...
    async def test_ignores_non_text_message(self):
        import bot
