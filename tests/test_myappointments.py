...
async def _run_cmd(appts, officials, chat_id=111, username="requester"):
    ...
    with patch("storage.get_appointments", side_effect=_fake_get), \
        ...
        await cmd_myappointments(upd, ctx)
    return upd.message.reply_text.call_args[0][0]


class TestMyAppointments:

    async def test_no_appointments_message(self):
        text = await _run_cmd([], _make_officials())
        assert "no appointments" in text.lower()

    async def test_requester_sees_their_appointment(self):
        appt = _make_appt(user_chat_id=111)
        text = await _run_cmd([appt], _make_officials(), chat_id=111)
        assert "APPT001" in text

    async def test_requester_sees_official_as_counterparty(self):
        appt = _make_appt(user_chat_id=111, official_name="Pastor Test")
        text = await _run_cmd([appt], _make_officials(), chat_id=111)
        assert "Pastor Test" in text

    async def test_requester_does_not_see_others_appointments(self):
        mine = _make_appt(appt_id="MINE", user_chat_id=111)
        other = _make_appt(appt_id="OTHER", user_chat_id=222)
        text = await _run_cmd([mine, other], _make_officials(), chat_id=111)
        assert "MINE" in text
        assert "OTHER" not in text

    async def test_official_sees_assigned_appointment_by_chat_id(self):
        appt = _make_appt(user_chat_id=111, official_id="off1")
        text = await _run_cmd([appt], _make_officials(chat_id=999), chat_id=999, username="pastor")
        assert "APPT001" in text

    async def test_official_sees_requester_as_counterparty(self):
    ...
    async def test_official_matched_by_telegram_username(self):
        appt = _make_appt(user_chat_id=111, official_id="off1")
        officials = _make_officials(chat_id=None, username="OneMorah")
        text = await _run_cmd([appt], officials, chat_id=555, username="OneMorah")
        assert "APPT001" in text

    async def test_all_statuses_shown(self):
        ...
        text = await _run_cmd([confirmed, cancelled, declined], _make_officials(), chat_id=111)
        assert "C1" in text and "X1" in text and "D1" in text

    async def test_status_label_present(self):
        appt = _make_appt(status="cancelled", user_chat_id=111)
        text = await _run_cmd([appt], _make_officials(), chat_id=111)
        assert "cancelled" in text

    async def test_upcoming_and_past_sections(self):
        ...
        old = _make_appt(appt_id="OLD", user_chat_id=111,
                         when=(datetime.now(TZ) - timedelta(days=10)).replace(microsecond=0))
        text = await _run_cmd([future, old], _make_officials(), chat_id=111)
        assert "*Upcoming:*" in text
        assert "*Past:*" in text

    async def test_only_upcoming_section_when_no_past(self):
        future = _make_appt(appt_id="FUT", user_chat_id=111,
                            when=(datetime.now(TZ) + timedelta(days=10)).replace(microsecond=0))
        text = await _run_cmd([future], _make_officials(), chat_id=111)
        assert "*Upcoming:*" in text
        assert "*Past:*" not in text

    async def test_formatted_datetime_shown(self):
        appt = _make_appt(user_chat_id=111,
    ...
    async def test_user_who_is_both_party_sees_appointment_once(self):
