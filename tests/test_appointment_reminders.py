...
async def _run_job(appts):
    ...
    with patch("storage.get_appointments", side_effect=_get), \
        ...
        await ha.appointment_reminder_job(ctx)
    return ctx, saved


class TestReminderTiming:
    async def test_nothing_due_far_out(self):
        ctx, saved = await _run_job([_appt(hours_ahead=48)])
        ctx.bot.send_message.assert_not_awaited()
        assert "appts" not in saved

    async def test_24h_stage_sends_to_both_parties(self):
        ctx, saved = await _run_job([_appt(hours_ahead=20)])
    ...
    async def test_2h_stage_sends(self):
        ctx, saved = await _run_job([_appt(hours_ahead=1.5, reminders={"24h": [111, 999]})])
        sent_to = {c.args[0] for c in ctx.bot.send_message.await_args_list}
        assert sent_to == {111, 999}
        assert sorted(saved["appts"][0]["reminders_sent"]["2h"]) == [111, 999]

    async def test_close_booking_sends_only_closest_stage(self):
        # Booked 1h ahead: both stages due at once → only the 2h reminder is
        # sent; the 24h stage is marked superseded without messages.
        ctx, saved = await _run_job([_appt(hours_ahead=1)])
    ...
    async def test_past_appointment_ignored(self):
        ctx, saved = await _run_job([_appt(hours_ahead=-1)])
        ctx.bot.send_message.assert_not_awaited()


class TestReminderIdempotency:
    async def test_already_sent_stage_not_resent(self):
        ctx, saved = await _run_job(
            [_appt(hours_ahead=20, reminders={"24h": [111, 999]})])
        ctx.bot.send_message.assert_not_awaited()
        assert "appts" not in saved

    async def test_partial_failure_retries_only_missing(self):
        # Official (999) already got the 24h reminder; only the user is sent.
        ctx, saved = await _run_job([_appt(hours_ahead=20, reminders={"24h": [999]})])
        sent_to = [c.args[0] for c in ctx.bot.send_message.await_args_list]
        assert sent_to == [111]
        assert sorted(saved["appts"][0]["reminders_sent"]["24h"]) == [111, 999]

    async def test_send_failure_leaves_recipient_pending(self):
...
class TestReminderScope:
    async def test_pending_appointment_ignored(self):
        ctx, saved = await _run_job([_appt(hours_ahead=20, status="pending")])
        ctx.bot.send_message.assert_not_awaited()

    async def test_cancelled_appointment_ignored(self):
        ctx, saved = await _run_job([_appt(hours_ahead=20, status="cancelled")])
        ctx.bot.send_message.assert_not_awaited()

    async def test_reminder_mentions_counterparty(self):
        ctx, saved = await _run_job([_appt(hours_ahead=20)])
...
class TestRearmOnReconfirm:
    async def test_finalize_clears_reminder_state(self):
        ...
        with patch("storage.save_appointments", side_effect=_save), \
             patch("handlers.appointments.get_user_prefs", side_effect=_prefs), \
             patch("permissions.OFFICIALS", _officials()):
            await ha._finalize_appointment(ctx, appt, appts)
